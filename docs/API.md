...
When several sources are connected, an optional `sourceId` picks which one the organization imports and mirrors from (the first connected source is the default):

```json
{ "org": "my-org", "role": "member", "sourceId": "…" }
```
