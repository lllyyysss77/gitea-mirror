...
GitHub to Gitea is the default and the best tested path. If your code lives elsewhere, the same setup works with a [GitLab source](/use-cases/mirror-gitlab-to-gitea/) or a [Codeberg, Forgejo or Gitea source](/use-cases/backup-codeberg-forgejo-repositories/), both beta, with the difference that issues, pull requests and releases are only read from GitHub for now.

## Requirements

- Gitea Mirror deployed (Docker, [Proxmox LXC](/use-cases/proxmox-lxc-homelab/), [Unraid](/use-cases/unraid-github-backup/), or [Helm](/use-cases/deploy-with-helm-chart/))
...
- Pick an interval that matches your recovery point objective; `15–30 minutes` keeps an active GitHub org nearly live, while `2–4 hours` is fine for quieter repos. Short intervals cost less than you would expect, because [conditional requests make unchanged checks nearly free](/use-cases/github-api-rate-limits/).
...
- Export the `/api/events` endpoint (or the Activity Log CSV) periodically to archive sync history alongside the repositories themselves.
...
- Rate limits cropping up? Lengthen the sync interval or trim the metadata components you mirror—authenticated GitHub rate limits are per account, so extra tokens from the same user won't raise the ceiling.
...
- [How issues and PRs transfer](/use-cases/mirror-github-issues-pull-requests/) when metadata is enabled
- [Git LFS mirroring](/use-cases/mirror-git-lfs-repositories/) and the one failure mode to know about
- [Make Gitea primary](/use-cases/migrate-github-to-gitea/) when lockstep becomes a migration
