...
- Export the **Activity Log** as CSV or hit `/api/events` quarterly so you retain a human-friendly change log.
- Back up the `data/` directory (the SQLite database holds your configuration) alongside your disaster-recovery docs in case you need to rebuild the service.
...
1. Delete a test issue on GitHub.
...
- [Backup GitHub repositories](/use-cases/backup-github-repositories/) for the day-to-day version of this
- [Build a starred repo collection](/use-cases/starred-repos-collection/) to archive other people's work too
