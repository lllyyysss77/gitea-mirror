...
You should see `gitea-mirror.db` (and a `repo-backups/` directory once pre-sync backups run). Data lives under `/opt/gitea-mirror/data`; back it up or snapshot the underlying storage pool regularly. The mirrored repositories themselves live on your Gitea server.
...
- [Unraid instead of Proxmox?](/use-cases/unraid-github-backup/) The CA template does the same job there
- [Kubernetes cluster?](/use-cases/deploy-with-helm-chart/) The Helm chart is the cluster-native path
- [What to configure once it boots](/use-cases/backup-github-repositories/)
