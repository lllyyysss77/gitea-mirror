...
2. Toggle **Automatic syncing** on and choose an interval that matches your recovery point objective (start with `30 minutes` for active teams, stretch to `12 hours` for archives). Frequent checks are cheap: [unchanged repositories cost almost no API budget](/use-cases/github-api-rate-limits/).
...
- Run `bun run manage-db check` during maintenance windows to verify the database and migrations are healthy.
...
- Back up the `data/` directory alongside your documentation—the SQLite database holds your configuration, so restoring it rebuilds the mirror quickly if you need to redeploy.
...
- [The base backup playbook](/use-cases/backup-github-repositories/) this automation builds on
- [Keep Gitea in lockstep with GitHub](/use-cases/sync-github-to-self-hosted-gitea/)
- [Per-repository mirror options](/use-cases/mirror-git-lfs-repositories/) when one repo needs different settings
