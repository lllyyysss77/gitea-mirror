...
This playbook covers the default path, GitHub into Gitea. Since 3.30 the source can also be [GitLab](/use-cases/mirror-gitlab-to-gitea/) or [Codeberg, Forgejo and Gitea](/use-cases/backup-codeberg-forgejo-repositories/), and since 3.31 the destination can be Forgejo, or [GitHub or GitLab](/use-cases/mirror-to-github-or-gitlab/) through a push engine. Those paths are marked beta in the settings.
...
Wait for the `Runtime server listening` line in the logs before moving on.
...
2. Log in to Gitea and create an access token for an admin/owner account with `write:repository` and `write:organization` (the second scope lets the mirror create destination organizations).
...
4. Complete the setup wizard:
   ...
   - Leave the sync interval at the default (daily) to start; you can shorten it later.
...
- Watch disk usage on your Gitea host—that's where the mirrored repos and LFS blobs actually live. Gitea Mirror's own `data/` volume stays small (SQLite database plus optional pre-sync backup bundles).
...
The mirrored repositories live in your Gitea instance's storage—Gitea does the actual mirroring. Gitea Mirror's `data/` directory holds its SQLite database (configuration, job history) and optional pre-sync backup bundles. Restore code by cloning from Gitea; restore the mirror service by moving the `data/` volume to a fresh deployment and signing back in.

## Related guides

- [Automate the schedule](/use-cases/github-backup-automation/) so backups run without you
- [GitHub API rate limits](/use-cases/github-api-rate-limits/) and why frequent syncs stay cheap
- [Migrate to Gitea entirely](/use-cases/migrate-github-to-gitea/) when a backup grows into an exit
- [Compare backup tools](/comparison/github-backup-tools/) if you are still choosing an approach
