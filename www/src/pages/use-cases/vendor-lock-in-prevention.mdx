...
### 5. Keep a second copy on a different host

Since 3.31 the destination does not have to be Gitea. **Forgejo** is a named destination, and **GitHub** and **GitLab** are push destinations (all three beta). A second Gitea Mirror account with a GitLab group as its destination gives you a copy on a different company's infrastructure at no extra cost, and the push engine keeps it identical to the source on every sync. Each account has one source and one destination, so run one account per direction. See [Mirror to GitHub or GitLab](/use-cases/mirror-to-github-or-gitlab/).
...
- Run **Reconcile with destination** from the Repository Cleanup card now and then. It lists mirrors on Gitea the database does not know about and rows whose mirror is gone, and fixes both on request without deleting anything. See [Adopt existing Gitea mirrors](/use-cases/adopt-existing-gitea-mirrors/).
...
- [The full migration guide](/use-cases/migrate-github-to-gitea/) for when the exit hatch gets used
- [Compare backup tools](/comparison/github-backup-tools/) to sanity-check this approach against the alternatives
